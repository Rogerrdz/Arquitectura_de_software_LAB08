...
Este repositorio ya incluye una implementación base alineada con el laboratorio:
- `infra/` con root module (`providers.tf`, `main.tf`, `variables.tf`, `outputs.tf`, `cloud-init.yaml`, `backend.hcl.example`, `env/dev.tfvars`, `env/prod.tfvars`).
- `modules/` con módulos reutilizables: `vnet`, `lb`, `compute`.
- `.github/workflows/terraform.yml` con `fmt`, `validate`, `plan` en PR y `apply` manual con OIDC.
- `docs/diagramas.md` y `docs/reflexion-tecnica-template.md` para apoyar entregables.

### Checklist para cumplir al 100
1. Completa `infra/env/dev.tfvars` con tu alias, fecha y `allow_ssh_from_cidr` real (`X.X.X.X/32`).
2. Crea `infra/backend.hcl` a partir de `infra/backend.hcl.example` con tu Storage Account real.
3. Configura secretos del repositorio en GitHub: `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`.
4. Ejecuta flujo local: `init`, `fmt`, `validate`, `plan`, `apply`.
5. Verifica balanceo con `curl` repetido al `lb_public_ip` y captura hostnames alternando.
6. Adjunta diagramas y reflexión técnica en la entrega.
7. Ejecuta `terraform destroy -var-file=env/dev.tfvars` al finalizar.

Guía práctica detallada paso a paso: `docs/paso-a-paso-100.md`.
